...
if($is_searching){
    $search = "SELECT * FROM payments WHERE admission LIKE ? AND dept= 'accessories' ";
...
//pagination
//COLLECTINg OF THE NUMBER OF STUDENTS WITH boarding FEES
$result_per_page = 1;
$current_page = $_GET['page'] ?? 1;


$stmt = "SELECT COUNT(*) FROM payments WHERE dept='accessories' ";
...
$sql = "SELECT * FROM payments WHERE dept='accessories' LIMIT $offset, $result_per_page";
...
<head>
    ...
    <title>accessories department online payments</title>
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">dashboard</span>accessories department online payments</h2>
        <div class="links">
    ...
    <div class="searchoutput">
        ...
        <table class="table">
            ...
            <th>date</th>
            <th>amount paid</th>
            <th>accessories payment message</th>
            <?php
            ...
            <tr>
                ...
                <td><?php echo "{$info['mpesa_message']}"?></td>
            </tr>
            <?php 
            }
            }elseif (empty($info)) {
                // Message when a search was attempted but returned zero results
                echo '<tr><td colspan="11">No student found matching the admission: ' . htmlspecialchars($input) .' who has boarding boarding fees'. '</td></tr>';
            } else {
    ...
    <div class="details">
        <h2>students that have settled their boarding debts online</h2>
        <table>
            <th>name</th>
            <th>admission</th>
            <th>created_at</th>

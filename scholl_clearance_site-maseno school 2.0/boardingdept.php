...
$Debt = $info['boardingitemsdamaged'];
$Debt_amount = $info['boardingitemsvalue']; 
$Dept_status = $info['boardingstatus'];
...
if (isset($_GET['choise']) && $_GET['choise'] == "payPhysically") {
    ...
    $boarding_data = $phy["boardingitemsdamaged"] ?? "Missing Items";
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET boardingstatus = 'pending_physical' WHERE admission = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "UPDATE physicall SET username = ?, year = ?, boarding = ?, boardingstatus = 'pending_physical_payment' WHERE admission = ?"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $year_data, $boarding_data, $admission_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        } else {
            ...
            $upsert_stmt->bind_param("ssss", $username_data, $admission_data, $year_data, $boarding_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        }
        ...
        $_SESSION['physicall_payment_prompt'] = "You have selected physical payment. Please bring the following on your clearance day: **" . htmlspecialchars($info['boardingitemsdamaged'] ?? 'Missing Item Details') . "**";
...
if(isset($_GET['choise']) && isset($_GET['choise']) == "noDebt"){
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET boardingstatus = 'cleared' WHERE admission = ? ");
    $stmt->bind_param("s", $admission_data);
    $stmt->execute();
    $stmt->close();
...
<head>
    ...
    <title>Boarding Clearance</title>
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">hotel</span>boarding Clearance</h2>
        <div class="links">
    ...
    <div class="details">
        <div class="table">
            ...
            <?php if($Dept_status == "uncleared" && $Debt != "none"):?>
                <h3 id="debt"><a href="mblpmt.php?dept=<?php echo urlencode("boarding")?>"><span class="material-symbols-outlined">payments</span>Pay Online (M-Pesa)</a></h3>
            ...
            <?php 
                $status = $info['boardingstatus'] ?? 'uncleared'; 
                if ($status == 'cleared'): 
            ?>

...
if($is_searching){
    $search = "SELECT * FROM physicall WHERE admission LIKE ? AND labstatus = 'pending_physical_payment' ";
...
$stmt = "SELECT COUNT(*) FROM physicall WHERE labstatus='pending_physical_payment' ";
...
$sql = "SELECT * FROM physicall WHERE labstatus='pending_physical_payment' LIMIT $offset, $result_per_page";
...
<head>
    ...
    <title>lab department pending_physical_payment</title>
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">dashboard</span>pending_physical_payment lab department</h2>
        <div class="links">
    ...
    <div class="searchoutput">
        ...
        <table class="table">
            ...
            <th>admission</th>
            <th>year</th>
            <th>damaged laboratory items </th>
            <th>lab fee </th>
            <th>lab status</th>
            <?php
            ...
            <tr>
                ...
                <td><?php echo "{$info['lab']}"?></td>
                <td><?php echo "{$info['labfee']}"?></td>
                <td><?php echo "{$info['labstatus']}"?></td>
            </tr>
            <?php 
            }
            }elseif (empty($info)) {
                // Message when a search was attempted but returned zero results
                echo '<tr><td colspan="11">No student found matching the admission: ' . htmlspecialchars($input) .' who has pending_physical_payment lab fees'. '</td></tr>';
            } else {
    ...
    <div class="details">
        <h2>students with pending_physical_payment lab fees</h2>
        <table>
            <th>name</th>
            <th>admission</th>
            <th>year</th>

...
$Debt = $info['gamesitemslost'];
$Debt_amount = $info['gamesitemvalue']; 
$Dept_status = $info['gamesstatus'];
...
if (isset($_GET['choise']) && $_GET['choise'] == "payPhysically") {
    ...
    $games_data = $phy["gamesitemslost"] ?? "Missing Items";
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET gamesstatus = 'pending_physical' WHERE admission = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "UPDATE physicall SET username = ?, year = ?, games = ?, gamesstatus = 'pending_physical_payment' WHERE admission = ?"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $year_data, $games_data, $admission_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        } else {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "INSERT INTO physicall (username, admission, year, games, gamesstatus) 
                VALUES (?, ?, ?, ?, 'pending_physical_payment')"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $admission_data, $year_data, $games_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        }
        ...
        $_SESSION['physicall_payment_prompt'] = "You have selected physical payment. Please bring the following on your clearance day: **" . htmlspecialchars($info['gamesitemslost'] ?? 'Missing Item Details') . "**";
    ...
    header("location: gamesdept.php");
    exit();
}



//for student with no debts
if(isset($_GET['choise']) && isset($_GET['choise']) == "noDebt"){

    $admission_data = $phy["admission"] ?? $_SESSION['admission'] ?? "N/A";

    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET gamesstatus = 'cleared' WHERE admission = ? ");
    $stmt->bind_param("s", $admission_data);
    $stmt->execute();
    $stmt->close();
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">sports_soccer</span>games Clearance</h2>
        <div class="links">
    ...
    <div class="details">
        <div class="table">
            ...
            <h4>
                Lost games Items
                <ol>
                    <li><?php echo htmlspecialchars($info['gamesitemslost'] ?? 'None found.') ?></li>
                </ol>
            </h4>
            ...
            <h4>
                Item's Value
                <ol>
                    <li><?php echo htmlspecialchars($info['gamesitemvalue'] ?? '0.00') ?></li>
                </ol>
            </h4>
            
     

            <!-- clearance button for student s with no debts NOTE (only displays if the user has no debts) -->
            <?php if($Debt == "none" && $Debt_amount = "0 "):?>
                <h3 id="debt"><a href="?choise=<?php echo urlencode("noDebts")?>"><span class="material-symbols-outlined">clear_all</span>looks like you have no outstandig debts, click here to clear.</a></h3>
            <?php endif;?>

            <!-- displays if the user has outstanding debts NOTE (only displays if the user ha debts) -->
            <?php if($Dept_status == "uncleared" && $Debt != "none"):?>
                <h3 id="debt"><a href="mblpmt.php?dept=<?php echo urlencode("games")?>"><span class="material-symbols-outlined">payments</span>Pay Online (M-Pesa)</a></h3>
            ...
            <?php 
                $status = $info['gamesstatus'] ?? 'uncleared'; 
                if ($status == 'cleared'): 
            ?>
            ...
            <h2><span class="material-symbols-outlined">event_available</span>Availability: <?php echo htmlspecialchars($info['gamesavailability'] ?? 'Check with Department') ?></h2>

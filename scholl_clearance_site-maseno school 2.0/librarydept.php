...
$Debt = $info['bookslost'];
$Debt_amount = $info['bookmarketvalue']; 
$Dept_status = $info['librarystatus'];
...
if (isset($_GET['choise']) && $_GET['choise'] == "payPhysically") {
    ...
    $library_data = $phy["bookslost"] ?? "Missing Items";
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET librarystatus = 'pending_physical' WHERE admission = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "UPDATE physicall SET username = ?, year = ?, bookname = ?, libstatus = 'pending_physical_payment' WHERE admission = ?"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $year_data, $library_data, $admission_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        } else {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "INSERT INTO physicall (username, admission, year, bookname, libstatus) 
                VALUES (?, ?, ?, ?, 'pending_physical_payment')"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $admission_data, $year_data, $library_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        }
        ...
        $_SESSION['physicall_payment_prompt'] = "You have selected physical payment. Please bring the following on your clearance day: **" . htmlspecialchars($info['bookslost'] ?? 'Missing Item Details') . "**";
    ...
    header("location: librarydept.php");
    exit();
}
...
if(isset($_GET['choise']) && isset($_GET['choise']) == "noDebt"){
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET librarystatus = 'cleared' WHERE admission = ? ");
    $stmt->bind_param("s", $admission_data);
    $stmt->execute();
    $stmt->close();
...
if (isset($_GET['online_payment_success']) && $_GET['online_payment_success'] == "true") {
    ...
    if ($admission_data != "N/A") {
        ...
        $stmt = $connecting_to_the_database->prepare(
            "UPDATE studentgeneraldata SET librarystatus = 'cleared' WHERE admission = ?"
        );
        $stmt->bind_param("s", $admission_data);
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">book_2</span>library Clearance</h2>
        <div class="links">
    ...
    <div class="details">
        <div class="table">
            ...
            <h4>
                Lost library Items
                <ol>
                    <li><?php echo htmlspecialchars($info['bookslost'] ?? 'None found.') ?></li>
                </ol>
            </h4>
            ...
            <h4>
                Item's Value
                <ol>
                    <li><?php echo htmlspecialchars($info['bookmarketvalue'] ?? '0.00') ?></li>
                </ol>
            </h4>
            ...
            <?php if($Dept_status == "uncleared" && $Debt != "none"):?>
                <h3 id="debt"><a href="mblpmt.php?dept=<?php echo urlencode("library")?>"><span class="material-symbols-outlined">payments</span>Pay Online (M-Pesa)</a></h3>
            ...
            <?php 
                $status = $info['librarystatus'] ?? 'uncleared'; 
                if ($status == 'cleared'): 
            ?>

...
if($is_searching){
    // CHANGE 1: Table name changed to 'physicall'
    $search = "SELECT * FROM physicall WHERE admission LIKE ? AND accessoriesstatus = 'pending_physical_payment' "; 
...
//pagination
//COLLECTINg OF THE NUMBER OF STUDENTS WITH PENDING PHYSICAL PAYMENT ACCESSORIES
$result_per_page = 1;
$current_page = $_GET['page'] ?? 1;


// CHANGE 2: Table name changed to 'physicall'
$stmt = "SELECT COUNT(*) FROM physicall WHERE accessoriesstatus='pending_physical_payment' ";
...
// CHANGE 3: Table name changed to 'physicall'
$sql = "SELECT * FROM physicall WHERE accessoriesstatus='pending_physical_payment' LIMIT $offset, $result_per_page";
...
<head>
    ...
    <title>accessories department pending physical payment</title>
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">dashboard</span>pending physical payment accessories department</h2>
        <div class="links">
    ...
    <div class="searchoutput">
        ...
        <table class="table">
            ...
            <tr>
                ...
                <td><?php echo "{$info['accessories']}"?></td>
                <td><?php echo "{$info['accessoriesstatus']}"?></td>
            </tr>
            <?php 
            }
            }elseif (empty($info) && $is_searching) {
                // Message when a search was attempted but returned zero results
                echo '<tr><td colspan="11">No student found matching the admission: ' . htmlspecialchars($input) .' who has pending physical payment for accessories'. '</td></tr>';
            } else {
    ...
    <div class="details">
        <h2>students with pending physical payment for accessories</h2>
        <table>
            <th>name</th>
            <th>admission</th>
            <th>year</th>

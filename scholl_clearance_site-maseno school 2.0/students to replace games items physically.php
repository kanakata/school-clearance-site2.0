...
if($is_searching){
    $search = "SELECT * FROM physicall WHERE admission LIKE ? AND gamesstatus = 'pending_physical_payment' ";
...
$stmt = "SELECT COUNT(*) FROM physicall WHERE gamesstatus='pending_physical_payment' ";
...
$sql = "SELECT * FROM physicall WHERE gamesstatus='pending_physical_payment' LIMIT $offset, $result_per_page";
...
<head>
    ...
    <title>games department pending_physical_payment</title>
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">dashboard</span>pending_physical_payment games department</h2>
        <div class="links">
    ...
    <div class="searchoutput">
        ...
        <table class="table">
            ...
            <tr>
                ...
                <td><?php echo "{$info['games']}"?></td>
                <td><?php echo "{$info['gamesstatus']}"?></td>
            </tr>
            <?php 
            }
            }elseif (empty($info)) {
                // Message when a search was attempted but returned zero results
                echo '<tr><td colspan="11">No student found matching the admission: ' . htmlspecialchars($input) .' who has pending_physical_payment games fees'. '</td></tr>';
            } else {
    ...
    <div class="details">
        <h2>students with pending_physical_payment games fees</h2>
        <table>
            <th>name</th>
            <th>admission</th>
            <th>year</th>

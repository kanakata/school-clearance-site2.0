...
$Debt = $info['unpaidaccessories'];
$Debt_amount = $info['unpaidaccessoriesvalue']; 
$Dept_status = $info['accessoriesstatus'];
...
if (isset($_GET['choise']) && $_GET['choise'] == "payPhysically") {
    ...
    $accessories_data = $phy["unpaidaccessories"] ?? "Missing Items";
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET accessoriesstatus = 'pending_physical' WHERE admission = ?");
...
if(isset($_GET['choise']) && isset($_GET['choise']) == "noDebt"){
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET accessoriesstatus = 'cleared' WHERE admission = ? ");
    $stmt->bind_param("s", $admission_data);
    $stmt->execute();
    $stmt->close();

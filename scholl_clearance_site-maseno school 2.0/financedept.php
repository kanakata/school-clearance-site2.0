...
// check if the student has  no debt
$Debt_amount = $info['feebalance']; 
$Dept_status = $info['feestatus'];
...
if (isset($_GET['choise']) && $_GET['choise'] == "payPhysically") {
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET feestatus = 'pending_physical' WHERE admission = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "UPDATE physicall SET username = ?, year = ?, feebalance = ?, mode = 'payment_to_be_paid_by_cheque', feestatus = 'pending_physical_payment' WHERE admission = ?"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $year_data, $Debt_amount, $admission_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        } else {
            ...
            $upsert_stmt = $connecting_to_the_database->prepare(
                "INSERT INTO physicall (username, admission, year, accessories, feestatus) 
                VALUES (?, ?, ?, ?, 'pending_physical_payment')"
            );
            $upsert_stmt->bind_param("ssss", $username_data, $admission_data, $year_data, $accessories_data);
            $upsert_stmt->execute();
            $upsert_stmt->close();
        }
        ...
        $_SESSION['physicall_payment_prompt'] = "You have selected physical payment. Please bring the following on your clearance day: **" . htmlspecialchars(" a cheque worth ") . "sh" . $Debt_amount . "**";
...
if(isset($_GET['choise']) && isset($_GET['choise']) == "noDebt"){
    ...
    $stmt = $connecting_to_the_database->prepare("UPDATE studentgeneraldata SET feestatus = 'cleared' WHERE admission = ? ");
    $stmt->bind_param("s", $admission_data);
    $stmt->execute();
    $stmt->close();
...
if (isset($_GET['online_payment_success']) && $_GET['online_payment_success'] == "true") {
    ...
    if ($admission_data != "N/A") {
        ...
        $stmt = $connecting_to_the_database->prepare(
            "UPDATE studentgeneraldata SET feestatus = 'cleared' WHERE admission = ?"
        );
        $stmt->bind_param("s", $admission_data);
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">finance</span>fee Clearance</h2>
        <div class="links">
    ...
    <div class="details">
        <div class="table">
            ...
            <h4>
                Fee balance value
                <ol>
                    <li><?php echo htmlspecialchars($info['feebalance'] ?? '0.00') ?></li>
                </ol>
            </h4>
            ...
            <?php if($Debt_amount == "0" && $Dept_status == "uncleared"):?>
            ...
            <?php if($Debt_amount != "0" && $Dept_status == "uncleared"):?>
                <h3 id="debt"><a href="mblpmt.php?dept=<?php echo urlencode("finance")?>"><span class="material-symbols-outlined">payments</span>Pay Online (M-Pesa)</a></h3>
                
                <h3 id="debt"><a href="?choise=<?php echo urlencode("payPhysically")?>"><span class="material-symbols-outlined">footprint</span>Pay Physically using a cheque (Mark Intention)</a></h3>
            <?php endif;?>    
            

            <!-- Check if user is cleared -->
            <?php 
                $status = $info['feestatus'] ?? 'uncleared'; 
                if ($status == 'cleared'): 
            ?>
            ...
            <!-- Check if payment is pending physical resolution -->
            <?php 

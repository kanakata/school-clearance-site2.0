...
if($is_searching){
    $search = "SELECT * FROM studentgeneraldata WHERE admission LIKE ? AND librarystatus = 'uncleared' ";
...
$stmt = "SELECT COUNT(*) FROM studentgeneraldata WHERE librarystatus='uncleared' ";
...
$sql = "SELECT * FROM studentgeneraldata WHERE librarystatus='uncleared' LIMIT $offset, $result_per_page";
...
<head>
    ...
    <title>library department uncleared</title>
...
<body>
    <nav>
        <h2><span class="material-symbols-outlined">dashboard</span>uncleared library department</h2>
        <div class="links">
    ...
    <div class="searchoutput">
        ...
        <table class="table">
            ...
            <tr>
                ...
                <td><?php echo "{$info['bookslost']}"?></td>
                <td><?php echo "{$info['librarystatus']}"?></td>
            </tr>
            <?php 
            }
            }elseif (empty($info)) {
                // Message when a search was attempted but returned zero results
                echo '<tr><td colspan="11">No student found matching the admission: ' . htmlspecialchars($input) .' who has uncleared library fees'. '</td></tr>';
            } else {
    ...
    <div class="details">
        <h2>students with uncleared library fees</h2>
        <table>
            <th>name</th>
            <th>admission</th>
            <th>year</th>
